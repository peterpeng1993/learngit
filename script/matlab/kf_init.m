...
%% 状态转移矩阵A
    kf_params.A = eye(4) + diag(ones(1, 2), 2); % 和线性系统的预测机制有关，
    %这里的线性系统是上一刻的位置加上速度等于当前时刻的位置，而速度本身保持不变
    %%预测噪声协方差矩阵Q：假设预测过程上叠加一个高斯噪声，协方差矩阵为Q
    %大小取决于对预测过程的信任程度。比如，假设认为运动目标在y轴上的速度可能不匀速，
    %那么可以把这个对角矩阵的最后一个值调大。有时希望出来的轨迹更平滑，可以把这个调更小
    kf_params.Q = diag(ones(4, 1) * 0.001); 
    
%% 观测矩阵H：z = H * x
    kf_params.H = eye(2, 4); % 这里的状态是（坐标x， 坐标y， 速度x， 速度y），
    %观察值是（坐标x， 坐标y），所以H = eye(2, 4)
    
%% 观测噪声协方差矩阵R：假设观测过程上存在一个高斯噪声，协方差矩阵为R
    kf_params.R = diag(ones(2, 1) * 2); %diag(v,(k))提取向量 v中的数值，构成一个n+abs(k)的方阵，k>0表示在主对角线上方，k<0表示在主对角线下方。
    %大小取决于对观察过程的信任程度。
    %比如，假设观测结果中的坐标x值常常很准确，那么矩阵R的第一个值应该比较小
end